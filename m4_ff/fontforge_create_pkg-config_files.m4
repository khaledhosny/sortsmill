...
__pkg_deps="${__pkg_deps} guile-2.0"
...
AC_SUBST([LIBFONTFORGE_PKGCONFIG_LIBS],["-L${libdir} -lsortsmillff_fontforge"])
...
AC_SUBST([LIBFONTFORGEEXE_PKGCONFIG_REQUIRES_PRIVATE],["libsortsmillff_fontforge ${__pkg_deps}"])
AC_SUBST([LIBFONTFORGEEXE_PKGCONFIG_LIBS],["-L${libdir} -lsortsmillff_fontforgeexe"])
AC_SUBST([LIBFONTFORGEEXE_PKGCONFIG_LIBS_PRIVATE],["-L${libdir} ${__private_exe_deps} ${__private_deps}"])

AC_CONFIG_FILES([libsortsmillff_fontforge.pc])
AC_CONFIG_FILES([libsortsmillff_fontforgeexe.pc])
])
