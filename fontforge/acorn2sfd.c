	...
	if ( kern_offset!=0 && !feof(file) && outline->metrics_n!=0 ) {
		...
		while ( (left=r_getshort(file))!=0 && !feof(file)) {
			...
			if ( !(flags&2) && !feof(file)) {
			    kern = xmalloc(sizeof(struct r_kern));
...
static void FindEncoding(SplineFont *sf,char *filename) {
    ...
    if ( pt!=NULL )
	*pt = '\0';
    otherdir = xmalloc(strlen(filename)+strlen("/../Encodings")+5);
    strcpy(otherdir,filename);
    strcat(otherdir,"/../Encodings");
    encfilename = xmalloc(strlen(otherdir)+strlen("base0encoding")+20);
...
return( NULL );
    ...
    outline.sf->map->map = xmalloc(outline.sf->glyphmax*sizeof(int32));
    outline.sf->map->backmap = xmalloc(outline.sf->glyphmax*sizeof(int32));
