...
(library (sortsmill usermenu)
  ...
  (define (set-menu-item-value! window-name menu-item key value)
    (let ([mi menu-item]
      ...
      (match key
        ('id *unspecified*)
        ('text (GTextInfo:text-set! ti (string->pointer-gc-utf8 v)))
        ('image (GTextInfo:image-set! ti (string->pointer-gc-utf8 v)))
        ...
        ('shortcut (GMenuItem:shortcut-set! mi (string->pointer-gc-utf8 v)))
  ...
  (define* (action-entry #:key text action
    ...
    (assert (or (eq? id #f) (string? id)))
    (assert (procedure? enabled))
    (assert (or (eq? shortcut #f) (string? shortcut)))
    (assert (or (eq? image #f) (string? image)))
    (assert (integer? foreground-color))
    (assert (integer? background-color))
    (assert (boolean? disabled?))
    (assert (boolean? checkable?))
    (assert (boolean? checked?))
    (assert (boolean? image-precedes-text?))
    (append `[(id        ,id)
              (text      ,text)
              (action    ,action)
              (enabled   ,enabled)
