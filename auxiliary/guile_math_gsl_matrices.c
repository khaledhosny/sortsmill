...
VISIBLE size_t
scm_matrix_dim1 (scm_t_array_handle *handlep)
{
  const size_t rank = scm_array_handle_rank (handlep);
  const scm_t_array_dim *dims = scm_array_handle_dims (handlep);
...
VISIBLE size_t
scm_matrix_dim2 (scm_t_array_handle *handlep)
{
  const size_t rank = scm_array_handle_rank (handlep);
  const scm_t_array_dim *dims = scm_array_handle_dims (handlep);
...
{
  size_t dim1 = scm_matrix_dim1 (handlep);
  size_t dim2 = scm_matrix_dim2 (handlep);
                  ...
                  SCM C)
{
  ...
  if (mpz_sgn (_beta) == 0)
  ...
  else
    {
      ...
      m_C = scm_matrix_dim1 (&handle_C);
      n_C = scm_matrix_dim2 (&handle_C);
    }
...
#define _FF_SCM_GMP_MATRIX_ELEMENTWISE_BINARY_OP(X, OP, FUNC, NON_CONFORMABILITY_MESSAGE) \
  ...
  {                                                                     \
    ...
    assert_c_exact_rank_1_or_2_array (who, B, &handle_B);               \
                                                                        \
    const size_t m_A = scm_matrix_dim1 (&handle_A);                     \
    const size_t n_A = scm_matrix_dim2 (&handle_A);                     \
                                                                        \
...
VISIBLE SCM
scm_gsl_linalg_SVD_golub_reinsch (SCM a)
{
  scm_t_array_handle handle_a;

  const char *who = "scm_gsl_linalg_SVD_golub_reinsch";
...
VISIBLE SCM
scm_gsl_linalg_SVD_modified_golub_reinsch (SCM a)
{
  scm_t_array_handle handle_a;

  const char *who = "scm_gsl_linalg_SVD_modified_golub_reinsch";
...
VISIBLE SCM
scm_gsl_linalg_SVD_jacobi (SCM a)
{
  scm_t_array_handle handle_a;

  const char *who = "scm_gsl_linalg_SVD_jacobi";
...
VISIBLE SCM
scm_gsl_linalg_SVD_solve (SCM U, SCM S, SCM V, SCM B)
{
  ...
  const char *who = "scm_gsl_linalg_SVD_solve";
...
init_guile_sortsmill_math_gsl_matrices (void)
{
  ...
  scm_c_define_gsubr ("gsl:svd-golub-reinsch-f64", 1, 0, 0,
                      scm_gsl_linalg_SVD_golub_reinsch);
  scm_c_define_gsubr ("gsl:svd-modified-golub-reinsch-f64", 1, 0, 0,
                      scm_gsl_linalg_SVD_modified_golub_reinsch);
  scm_c_define_gsubr ("gsl:svd-jacobi-f64", 1, 0, 0, scm_gsl_linalg_SVD_jacobi);
  scm_c_define_gsubr ("gsl:svd-solve-f64", 4, 0, 0, scm_gsl_linalg_SVD_solve);
