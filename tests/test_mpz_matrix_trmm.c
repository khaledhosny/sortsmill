...
my_main (int argc, char **argv)
{
  ...
  mpz_t alpha;
  mpz_init (alpha);
  scm_dynwind_mpz_clear (alpha);
  mpz_set_str (alpha, argv[7], 0);

  int k = (Side == CblasLeft) ? m : n;

  mpz_t A[k][k];
  mpz_matrix_init (k, k, A);
  scm_dynwind_mpz_matrix_clear (k, k, A);

  mpz_t B[m][n];
  mpz_matrix_init (m, n, B);
  scm_dynwind_mpz_matrix_clear (m, n, B);
  ...
  mpz_matrix_trmm (Side, Uplo, TransA, Diag, m, n, alpha, A, B);
  gsl_blas_dtrmm (Side, Uplo, TransA, Diag, mpz_get_d (alpha), &mA1.matrix,
                  &mB1.matrix);
  ...
  for (unsigned int i = 0; i < m; i++)
    for (unsigned int j = 0; j < n; j++)
      {
        gmp_printf ("B[%u][%u] = %lf\t%Zd\n", i, j, B1[i][j], B[i][j]);
        if (10000 * DBL_EPSILON < fabs (mpz_get_d (B[i][j]) - B1[i][j]))
          exit_status = 1;
      }
